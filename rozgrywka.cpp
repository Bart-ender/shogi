...
string rozgrywka::wstaw(string num, string poz){
    if(!e.numer(num) || !e.czy_pozycja(poz)) return "błędny format\n";
    int n = stoi(num);
...
string rozgrywka::przesun(string poz1, string poz2, string prom){
    if(!e.czy_pozycja(poz1) || !e.czy_pozycja(poz2)) return "błędny format\n";
 ...
 //   if(pion.get_symbol()=='⬜') return "na wybranym polu startowym nie ma pionka\n//";
    if(promuj){
        if(!e.promocja(g,p2)) return "nie można wykonać promocji na tym polu\n";
    }
    ...
    if(e.sprawdz_ruch(g,p1,p2,p)){
        ...
        if(bity.get_symbol()=='k') {
            if(g) return "Koniec gry gracz2 wygrał\n";
            return "Koniec gry gracz1 wygrał\n";
        }
        g^=1;
        return "";
    }
