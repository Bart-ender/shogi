...
plansza::plansza(){
    ...
    vector<string> pl = {r0,r1,r2, r3,r3,r3, r3,r7,r0};
...
void plansza::przesun(pionek p, pozycja poz, gracz *g){
    ...
    if(p2.get_symbol()!='⬜') g->dodaj_pionek(p2);
    board[poz.x][poz.y]=p;
    if(p.get_symbol()=='k'){
...
bool plansza::pion(pozycja poz1, pozycja poz2){
    ...
    for(int i=poz1.x+pomi;i!=poz2.x;i+=pomi){
        if(board[i][poz1.y].get_symbol()!='⬜') return true;
    }
    return false;
}
...
bool plansza::poziom(pozycja poz1, pozycja poz2){
    ...
    for(int i=poz1.y+pomi;i!=poz2.y;i++){
        if(board[poz1.x][i].get_symbol()!='⬜') return true;
    }
    return false;
}
...
bool plansza::skos(pozycja poz1, pozycja poz2){
    ...
    for(int i=pom1+poz1.x;i!=poz2.x;i+=pom1){
        if(board[i][y].get_symbol()!='⬜') return true;
        y+=pom2;
    }
    return false;
}
...
string plansza::wypisz(){
    string ret="";
    for(int i=0;i<9;i++){
        for(int j=0;j<9;j++) ret.push_back(board[i][j].get_symbol());
        ret.push_back('\n');
    }
    return ret;
}
